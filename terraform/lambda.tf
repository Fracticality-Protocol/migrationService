    ...
    variables = merge(
      {
        ...
        REDIS_CONNECTION_STRING         = "${aws_elasticache_replication_group.redis_cluster.primary_endpoint_address}:${try(aws_elasticache_replication_group.redis_cluster.port, 6379)}"
        REDIS_USE_TLS                   = "true"
      },
